...
import { toggleAddressChange } from '../state/actions/address.action';
...
export class LocationComponent {
  ...
  ngOnInit(){
    ...
    this.section$ = this.store.select((state: any)=>{
      ...
      const section: string[] = state.address.list[this.genericFormGroup.value.Province][this.genericFormGroup.value.District].map((section: any)=>section.section);
      return section
    })
  }
  provinceSelected(){
