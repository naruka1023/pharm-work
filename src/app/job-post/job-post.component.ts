  ...
  ngOnInit(){
    this.activatedRoute.paramMap.subscribe((id)=>{
      getDoc(doc(this.db, 'job-post', id.get('id')!)).then((job)=>{
        ...
        getDocs(query(collection(this.db, 'job-post'), where('Active', '==', true), where('OperatorUID', '==', this.profile.OperatorUID))).then((jobs)=>{
          this.urgentJobs = jobs.docs.map(job => {return {...job.data() as jobPostModel, custom_doc_id:job.id}})
