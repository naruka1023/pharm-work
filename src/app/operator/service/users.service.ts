...
import {
  ...
  removeRequestView,
  setRequestView,
} from '../state/actions/request-view.actions';
...
const client = algoliasearch(
  ...
  algoliaEnvironment.user_api_key
);
@Injectable({
  providedIn: 'root',
})
export class UsersService {
          ...
          if (key == 'WorkExperience') {
            ...
            if (key == 'preferredJobType') {
              query =
                key +
                ":'" +
                this.converter.getTitleFromCategorySymbol(newForm[key]) +
                "'";
            } else {
              query = key + ":'" + newForm[key] + "'";
            }
  ...
  async searchPharmaUsersUrgentByPreferredJobType(form: UserUrgentSearchForm) {
    ...
    if (newForm.urgentPreferredDay !== '') {
      query +=
        ' AND urgentPreferredDay:' + "'" + newForm.urgentPreferredDay + "'";
    }
    ...
    if (!newForm.nearbyFlag) {
      if (newForm.preferredUrgentSection !== '') {
        query +=
          ' AND preferredUrgentSection:' +
          "'" +
          newForm.preferredUrgentSection +
          "'";
      }
      if (newForm.preferredUrgentDistrict !== '') {
        query +=
          ' AND preferredUrgentDistrict:' +
          "'" +
          newForm.preferredUrgentDistrict +
          "'";
      }
      if (newForm.preferredUrgentProvince !== '') {
        query +=
          ' AND preferredUrgentProvince:' +
          "'" +
          newForm.preferredUrgentProvince +
          "'";
      }
    } else {
    ...
    let filter =
      "role:'เภสัชกร' AND studentFlag:false" +
      query +
      " AND preferredJobType:'" +
      this.converter.getTitleFromCategorySymbol('S') +
      "'";
  ...
  updateNotifications(notification: Partial<notificationContent>) {
    return updateDoc(
      doc(this.db, 'notification-archive', notification.notificationID!),
      notification
    );
  }
  ...
  getNotifications(userUID: string) {
    onSnapshot(
      query(
        collection(this.db, 'notification-archive'),
        ...
        return notification.docChanges().map((value) => {
          ...
          if (notificationArray.length > 0) {
            ...
            notificationArray.forEach((notification) => {
              finalNotificationPayload[notification.notificationID] =
                notification;
            });
            this.store.dispatch(
              addNotifications({
                notifications: finalNotificationPayload,
                size: size,
              })
            );
          }
      ...
      promises.push(
        getDocs(
          query(
            ...
            where('role', '==', 'เภสัชกร'),
            limit(5),
            where('studentFlag', '==', false),
            where(
              'preferredJobType',
              'array-contains',
              this.converter.getTitleFromCategorySymbol(
                placeHolder.categorySymbol
              )
            ),
  ...
  async getCountGroup(): Promise<aggregationCount> {
    const jobcount = await getCountFromServer(
      query(collection(this.db, 'job-post'), where('Active', '==', true))
    );
    const userPharmaCount = await getCountFromServer(
      query(collection(this.db, 'users'), where('role', '==', 'เภสัชกร'))
    );
    const userOperatorCount = await getCountFromServer(
      query(collection(this.db, 'users'), where('role', '!=', 'เภสัชกร'))
    );
    const result = {
  ...
  async getFavorites(userUID: string) {
    let docs = await getDocs(
      query(
        collection(this.db, 'favorite'),
        where('operatorUID', '==', userUID)
      )
    );
  ...
  getRequestView(operatorUID: string): Unsubscribe {
    return onSnapshot(
      query(
        collection(this.db, 'request-view'),
        where('operatorUID', '==', operatorUID)
      ),
      (values) => {
        values.docChanges().forEach((value) => {
          ...
          switch (requestView.type) {
            case 'added':
              this.getUserFromRequestView(requestView.payload).then(
                (requestView: requestView) => {
                  let requestViewList: requestViewList = {};
                  let key: string =
                    requestView.userUID + '-' + requestView.operatorUID;
                  requestViewList[key] = requestView;
                  this.store.dispatch(
                    setRequestView({ requestViewList: requestViewList })
                  );
                }
              );
              break;
            case 'removed':
              this.store.dispatch(
                removeRequestView({ requestView: requestView.payload })
              );
              break;
            case 'modified':
              this.store.dispatch(
                modifyRequestView({ requestView: requestView.payload })
              );
              break;
          }
        });
