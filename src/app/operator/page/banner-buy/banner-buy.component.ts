...
export class BannerBuyComponent {
  ...
  packageList:
  {
    name: string, description: string, priceID: string, price: number
  }[] = []
  breakingPoint = {
    1400: {
      slidesPerView: 3
  ...
  ngOnInit(){
    ...
    this.store.select((state: any)=>{
      ...
      Object.keys(packages).sort(sortAlphaNum).forEach((key)=>{
  ...
  uploadImage(){
    ...
    uploadBytes(ref(this.storage, 'users/' + this.currentUserUID + '/banner/' + this.productPackage), this.file).then(()=>{
      getDownloadURL(ref(this.storage, 'users/' + this.currentUserUID + '/banner/' + this.productPackage)).then((url: string)=>{
  ...
  loadFile(event: any){
    if(event !== undefined){
      ...
      if( currWidth !== 0 && currHeight !== 0){
        let dimensions = this.packageDimensions[this.productPackage].split(' * ')
        this.dimensionFlag = Number(dimensions[0]) == currWidth && Number(dimensions[1]) == currHeight
      }
      myImg.setAttribute('style','width:-webkit-fill-available; width:-moz-available');
    }
  }
  imageChosen(event: any){
      if(event.target.files.length > 0){
        this.placeholderImage = (window.URL ? URL : webkitURL).createObjectURL(event.target.files[0]);
