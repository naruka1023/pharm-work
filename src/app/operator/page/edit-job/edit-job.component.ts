...
export class EditJobComponent implements OnDestroy {
  constructor(
    ...
    private editJobService: JobService,
  ...
  ngOnInit() {
    ...
    this.urgency =
      this.route.snapshot.queryParamMap.get('urgency')! == 'false'
        ? false
        : true;
    ...
    this.subToDestroy = this.store
      .select((state: any) => {
        const newState = _.cloneDeep(
          state.createdJobs.JobPost
        ) as jobPostModel[];
      ...
      .subscribe((state: any) => {
        ...
        if (
          this.newJobForm.value.Salary.Suffix !== 'CorporateStructure' &&
          this.newJobForm.value.Salary.Suffix !== 'SalaryNumbers' &&
          this.newJobForm.value.Salary.Suffix !== 'Negotiable'
        ) {
  ...
  onUpdate() {
    ...
    if (this.newJobForm.invalid) {
      ...
      processedInfo = {
        ...processedInfo,
        Duration:
          this.newJobForm.value.timeStart +
          ' - ' +
          this.newJobForm.value.timeEnd,
      };
