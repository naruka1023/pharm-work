...
import {
  ...
  setCurrentUser,
  updatePackage,
} from '../state/actions/users.action';
...
export class LandingPageComponent implements AfterViewInit {
  ...
  @ViewChild('searchInputUrgent', { static: false })
  searchInput!: ElementRef<HTMLInputElement>;

  constructor(
    private activatedRoute: ActivatedRoute,
    private requestJobsComponent: RequestJobComponent,
    private operatorProfileComponent: OperatorProfileComponent,
  ...
  ngOnInit() {
    ...
    this.successCheckoutFlag = this.activatedRoute.snapshot.queryParamMap.get(
      'successCheckoutFlag'
    );
    this.cancelCheckoutFlag =
      this.activatedRoute.snapshot.queryParamMap.get('cancelCheckoutFlag');
        ...
        if (this.user.uid !== '' && this.subscribeFlag) {
          ...
          getDocs(
            query(
              ...
              where('name', 'in', ['A1', 'A2', 'A3', 'A4', 'B1', 'B2']),
              where('active', '==', true)
            )
          ).then((value) => {
            ...
            value.docs.forEach((innerDoc) => {
              ...
              getDocs(
                query(
                  collection(this.db, 'products', innerDoc.id, 'prices'),
                  where('active', '==', true)
                )
              ).then((price) => {
          ...
          getDocs(
            query(
              collection(this.db, 'users', this.user.uid, 'subscriptions'),
              where('status', 'in', [
                'trialing',
                'active',
                'incomplete',
    ...
    user(this.auth).subscribe((user) => {
      if (user) {
        ...
        this.store
          ...
          .subscribe((followers: any) => {
            ...
            if (this.followFlag) {
              ...
              this.userService
                .getNumberOfFollowers(followers.uid)
                .then((number: number) => {
                  const payload: any = {
                    followers: number,
                  };
        ...
        this.userService.getFavorites(user.uid).then((favorites1: any) => {
          this.userService
            .getListOfUsersFromFavorites(favorites1)
            .then((favorites) => {
              this.store.dispatch(
                setFavorites({ favorites: favorites as any })
              );
            });
        });
        onSnapshot(
          query(collection(this.db, 'users'), where('uid', '==', user.uid)),
          (snapshot) => {
      ...
      if (this.loginFlag) {
        toSend = {
          subject:
            this.user.uid +
            ' : ' +
            this.user.nameOfPerson +
            ' from company ' +
            this.user.companyName +
            ' have sent a message from PharmWork contact us form',
          html: emailContent,
        };
      } else {
        toSend = {
  ...
  appendAlert(message: any, type: any, i: number, link: string) {
    ...
    wrapper.innerHTML = [
      ...
      `   
          <div><img src=${
            message.image
          } width=150 height=150 class="me-3"/></div>  
          <div>
            <div class="mb-3">
              <b>${message.title}</b>
            </div>
  ...
  goToAddJob(event: any, urgencyFlag: boolean) {
    if (event.target.className.indexOf('question') == -1) {
      ...
      this.route
        .navigate(['/operator/add-new-jobs'], {
          queryParams: {
            urgency: urgencyFlag,
          },
        })
