...
export class ProfileHeaderComponent {
  ...
  constructor(
    ...
    private store: Store,
    private profileService: UtilService
  ) {}

  private storage: FirebaseStorage = this.firebaseService.storage;
  private firebaseApp: FirebaseApp = this.firebaseService.app;
      ...
      case 'pharma-profile':
        this.headerInformation = this.store.select((state: any) => {
          switch (this.route.snapshot.queryParamMap.get('pageType')!) {
            case 'recently-seen':
              this.profileInformation$ = state.recentlySeen.find(
                (user: UserPharma) => {
                  return (
                    user.uid == this.route.snapshot.queryParamMap.get('userUID')
                  );
                }
              );
              break;
            ...
            case 'request-jobs':
              let profileLinkFlag =
                this.route.snapshot.queryParamMap.get('profileLinkPage');
              if (profileLinkFlag == 'true') {
                this.profileInformation$ =
                  state.requestView[
                    this.route.snapshot.queryParamMap.get('userUID') +
                      '-' +
                      state.user.uid
                  ];
                ...
                let requestUID =
                  this.route.snapshot.queryParamMap.get('requestUID');
                this.profileInformation$ =
                  state.requestedJobs.JobRequests[jobUID!].users[
                    requestUID +
                      '-' +
                      this.route.snapshot.queryParamMap.get('userUID')
                  ];
            ...
            default:
              this.profileInformation$ =
                state.users.users[
                  this.route.snapshot.queryParamMap.get('categorySymbol')!
                ][this.route.snapshot.queryParamMap.get('pageType')!][
                  this.route.snapshot.queryParamMap.get('userUID')!
                ];
              break;
          }
    ...
    this.requestViewFlag$ = this.store.select((state: any) => {
      ...
      if (this.userUID !== '') {
        let requestView: requestView =
          state.requestView[this.result.uid + '-' + this.userUID];
    ...
    this.favoriteFlag$ = this.store.select((state: any) => {
      ...
      if (this.userUID !== '') {
        ...
        let favorite: Favorite =
          newState.Favorites[this.userUID + '-' + this.result.uid];
  ...
  getFavoritePayload() {
    return {
      operatorUID: this.userUID,
      user: this.result,
      favoriteUID: this.favoriteID,
    };
  }
  ...
  toggleFavorite() {
    if (this.localFlag === true) {
      ...
      this.favoriteLoadingFlag = true;
      this.userService
        .addFavorite(this.userUID, this.result.uid)
        .then((value) => {
      ...
      switch (cmd) {
            ...
            if (
              ...
              getDownloadURL(
                ...
                this.profileService.getUID(this.result.uid).then((docs) => {
                  this.profileService
                    .updateUserJobsCoverPhoto(docs, url, ele.scrollTop)
                    .then(() => {});
                });
              });
            } else {
              user.coverPhotoPictureUrl =
                this.result.coverPhotoPictureUrl.split('?t=')[0] +
                '?t=' +
                new Date().getTime();
              this.profileService
                .getUID(this.result.uid)
        ...
        case 'save':
          ...
          this.profileService.updateUser(user).then(() => {
            this.savePhotoLoadingFlag = false;
            this.store.dispatch(
              updateCoverPhotoOffset({ offset: ele.scrollTop })
            );
            this.editCoverPhotoFlag = true;
          });
          break;
  ...
  uploadProfilePicture() {
    ...
    listAll(ref(storage, 'users/' + this.userUID)).then((list: any) => {
      ...
      uploadString(storageRef, imgSrc, 'data_url').then(() => {
        if (croppedNotExists) {
          getDownloadURL(
            ref(this.storage, 'users/' + this.userUID + '/crop-profile')
          ).then((url: string) => {
            let user: Partial<User> = {
              uid: this.userUID,
            ...
            this.profileService.getUID(this.result.uid).then((docs) => {
              this.profileService
                .updateUserJobs(docs, url + '?t=' + new Date().getTime())
                .then(() => {});
            });
          });
        } else {
          ...
          profPic.src =
            this.result.cropProfilePictureUrl + '?t=' + new Date().getTime();
          this.store.dispatch(
  ...
  editProfilePictureClick($event: any, editExistingPhoto?: string) {
    ...
    if (editExistingPhoto === '') {
      ...
      getMetadata(
        ref(getStorage(), `users/${this.result.uid}/profile-picture`)
      ).then((metaData: any) => {
