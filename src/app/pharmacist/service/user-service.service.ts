...
import {
  ...
  requestViewList,
  User,
} from '../model/typescriptModel/users.model';
import {
  collection,
  deleteDoc,
  doc,
...
export class UserServiceService {
  ...
  private firestore: Firestore = this.firebaseService.firestore;
  ...
  async getCountGroup(): Promise<aggregationCount> {
    const jobcount = await getCountFromServer(
      query(collection(this.firestore, 'job-post'), where('Active', '==', true))
    );
    const userPharmaCount = await getCountFromServer(
      query(collection(this.firestore, 'users'), where('role', '==', 'เภสัชกร'))
    );
    const userOperatorCount = await getCountFromServer(
      query(
        collection(this.firestore, 'users'),
        where('role', '==', 'ผู้ประกอบการ')
      )
    );
    const result = {
  ...
  updateNotifications(notification: Partial<notificationContent>) {
    return updateDoc(
      doc(this.firestore, 'notification-archive', notification.notificationID!),
      notification
    );
  }
  getLimitBanner() {
    return getDocs(query(collection(this.firestore, 'banners'))).then(
      (value: any) => {
        let resultPayload = value.docs.map((row: any) => row.data());
        resultPayload = resultPayload[0];
        Object.keys(resultPayload).map((key) => {
          resultPayload[key] =
            resultPayload[key].length < limitBanner[key] ? false : true;
        });
  ...
  getNotifications(userUID: string) {
    onSnapshot(
      query(
        collection(this.firestore, 'notification-archive'),
  ...
  getRequestView(userUID: string) {
    onSnapshot(
      query(
        collection(this.firestore, 'request-view'),
        where('userUID', '==', userUID)
      ),
      (requestView) => {
        return requestView.docChanges().map((value) => {
          ...
          switch (requestViewPayload.type) {
            ...
            case 'removed':
              this.store.dispatch(
                removeRequestView({ requestView: requestViewPayload.payload })
              );
              break;
            case 'modified':
              this.store.dispatch(
                modifyRequestView({ requestView: requestViewPayload.payload })
              );
              break;
          }
          if (requestViewArray.length !== 0) {
            this.getListOfUsersFromRequestView(requestViewArray).then(
              (populatedRequestViews: requestView[]) => {
                ...
                this.store.dispatch(
                  setRequestView({ requestViewList: requestViewListPayload })
                );
              }
            );
          }
  ...
  confirmRequestView(requestStatus: requestView) {
    return updateDoc(
      doc(this.firestore, 'request-view', requestStatus.requestViewUID!),
      {
        status: 'Accepted',
      }
    );
  ...
  getListOfUsersFromRequestView(array: requestView[]): Promise<requestView[]> {
    ...
    array.forEach((requestView: requestView) => {
      promises.push(
        getDoc(doc(this.firestore, 'users', requestView.operatorUID))
      );
    });
