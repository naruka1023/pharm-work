...
import { Auth, sendEmailVerification, onAuthStateChanged } from 'firebase/auth';
...
export class LandingPageComponent {
  ...
  constructor(
    ...
    private pharmaProfileComponent: PharmaProfileComponent,
  ...
  appendAlert(message: any, type: any, i: number) {
    ...
    wrapper.innerHTML = [
      ...
      `   
        <div><img src=${
          message.image
        } width=100 height=100 class="notificationImage me-3"/></div>  
        <div>
          <div class="mb-3">
            <b>${message.title}</b>
          </div>
  ...
  toggleShare(jobPost: jobPostModel) {
    ...
    this.nameToShare =
      'https://public.pharm-work.com/job-post/' + this.idToShare;
    this.copy = 'Copy';
  ...
  onLoginClick() {
    this.operatorModal.hide();
    const triggerEl: any = document.querySelector(
      '#myTab button[data-bs-target="#urgent-job-pane-login"]'
    );
    triggerEl!.click();
  ...
  onRegisterClick() {
    const triggerEl: any = document.querySelector(
      '#myTab2 button[data-bs-target="#urgent-job-pane"]'
    );
    triggerEl!.click();
    this.operatorModal.hide();
    this.registerModal.show();
