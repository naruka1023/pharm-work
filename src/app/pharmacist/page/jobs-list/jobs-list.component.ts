...
export class JobsListComponent implements OnDestroy {
  ...
  ngOnInit() {
    ...
    this.count$ = this.store.select((state: any) => {
      const jobList: filterConditions = state.jobpost.JobPost.find(
        (res: any) => {
    ...
    this.content$ = this.store.select((state: any) => {
      ...
      this.brandToCategory =
        jobList.brandToCategory !== undefined ? jobList.brandToCategory : '';
  ...
  searchJobs() {
    ...
    if (
      this.urgentFilterForm.value.DateOfJob !== '' &&
      this.urgentFilterForm.value.DateOfJob !== undefined
    ) {
      if (this.urgentFilterForm.value.DateOfJob.indexOf(', ') == -1) {
        this.urgentFilterForm.patchValue({
          DateOfJob: [this.urgentFilterForm.value.DateOfJob],
        });
      } else {
        this.urgentFilterForm.value.DateOfJob =
          this.urgentFilterForm.value.DateOfJob.split(', ');
      }
    }
    if (
      this.urgentFilterForm.value.nearbyFlag &&
      this.urgentFilterForm.value.radius == ''
    ) {
  ...
  initializeSelector() {
    ...
    this.district$ = this.store.select((state: any) => {
      ...
      return Object.keys(
        state.address.list[this.urgentFilterForm.value.Location.Province]
      );
    });
    this.section$ = this.store.select((state: any) => {
      ...
      const section: string[] = state.address.list[
        this.urgentFilterForm.value.Location.Province
      ][this.urgentFilterForm.value.Location.District].map(
  ...
  reset() {
    ...
    this.jobPostService
      .getJobCategoryService(this.CategorySymbol, this.paginationIndex)
      .then((jobPosts) => {
        this.paginationIndex++;
