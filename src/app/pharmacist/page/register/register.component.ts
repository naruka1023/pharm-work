...
export class RegisterComponent implements AfterViewInit {
  ...
  @ViewChild('swiperFormPharma', { static: false })
  swiperFormPharma!: ElementRef<SwiperContainer>;
  @ViewChild('swiperFormStudent', { static: false })
  swiperFormStudent!: ElementRef<SwiperContainer>;

  swiperFormPharmaInstance!: Swiper;
  swiperFormStudentInstance!: Swiper;
  ...
  onSwiper() {
    this.swiperPharmaIndex = this.swiperFormPharmaInstance.activeIndex;
  }
  onSwiperStudent() {
    this.swiperStudentIndex = this.swiperFormStudentInstance.activeIndex;
  }
  ...
  addEducation() {
    ...
    let newValue: FormArray = this.registerFormStudent.controls[
      'educationHistory'
    ] as FormArray;
    newValue.push(educationToAdd);
    this.registerFormStudent.controls['educationHistory'].setValue(
      newValue.value
    );
  }
  ...
  initializeFormGroup() {
    ...
    this.registerFormPharmacist
      .get('employmentFlag')
      ?.valueChanges.subscribe((flag: any) => {
        if (flag) {
      ...
      ?.valueChanges.subscribe((profile: any) => {
        profile.forEach((pr: JobHistory) => {
          ...
          if (pr.dateStarted !== '' && pr.dateEnded !== '') {
            ...
            let years =
              Math.floor(dateDiff / 12) > 0
                ? Math.floor(dateDiff / 12) + ' ปี'
                : '';
  ...
  get pharmaFormEduData(): any {
    let entity = this.registerFormPharmacist.get(
      'educationHistory'
    ) as FormArray;
    return entity.controls;
  }
  ...
  get activeIndex(): any {
    return this.swiperFormPharmaInstance.activeIndex;
  }
  nextSlideStudent() {
    this.swiperFormStudentInstance.slideNext();
    this.studentForm = false;
  }
  backSlideStudent() {
    this.swiperFormStudentInstance.slidePrev();
  }
  nextSlide(studentFlag: boolean) {
    if (studentFlag) {
      this.nextSlideStudent();
    } else {
      this.swiperFormPharmaInstance.slideNext();
      let display = 'k';
      let display2 = 'k';
      switch (this.swiperFormPharmaInstance.activeIndex) {
        case 1:
          display = 'none';
          display2 = 'none';
  ...
  originSlide() {
    this.swiperFormPharmaInstance.slideTo(0);
  }
  backSlide() {
    this.swiperFormPharmaInstance.slidePrev();
    let display = 'k';
    let display2 = 'k';
    switch (this.swiperFormPharmaInstance.activeIndex) {
      case 0:
      ...
      case 'เภสัชกร':
        if (this.registerFormPharmacist.invalid) {
          ...
          if (!this.registerFormPharmacist.value.employmentFlag) {
            let jobHistoryList: JobHistory[] =
              this.registerFormPharmacist.get('jobHistory')?.value;
            jobHistoryList.forEach((jobHistory, index) => {
              if (jobHistory.workExperience == 0) {
                ...
                if (pr.dateStarted !== '' && pr.dateEnded !== '') {
                  let dateDiff = this.calculateDateDiff(
                    pr.dateStarted,
                    pr.dateEnded
                  );
                  let years =
                    Math.floor(dateDiff / 12) > 0
                      ...
                      : '';
                  let months =
                    dateDiff % 12 > 0 ? (+dateDiff % 12) + ' เดือน' : '';
                  pr.workExperience = years + ' ' + months;
                }
                pr.dateStarted = pr.dateStarted.replace('-', '/');
                pr.dateEnded = pr.dateEnded.replace('-', '/');
        ...
        newUser.preferredJobType = this.converter.objectToArray(
          newUser.preferredJobType
        );
        break;
      case 'ผู้ประกอบการ':
      ...
      this.userService
        ...
        .subscribe((res: any) => {
          if (res.result) {
            ...
            this.backSlide();
            this.errorMessage =
              'ชื่อหรือนามสกุลหรือเลขใบประกอบของท่านไม่ถูกต้องตามฐานข้อมูลรายชื่อผู้ประกอบวิชาชีพเภสัชกรรม';
          }
        });
    }
  }
      ...
      .then((user: any) => {
        ...
        if (newUser.role == 'เภสัชกร') {
          if (newUser.gender == 'ชาย') {
            promises.push(
              getDownloadURL(
                ref(this.storage, 'placeholder/male-pharma-profile.png')
              )
            );
          } else {
            promises.push(
              getDownloadURL(
                ref(this.storage, 'placeholder/female-pharma-profile.png')
              )
            );
          }
          promises.push(
            getDownloadURL(
              ref(this.storage, 'placeholder/pharma-cover-photo.png')
            )
          );
        } else {
          promises.push(
            getDownloadURL(
              ref(this.storage, 'placeholder/operator-profile.png')
            )
          );
          promises.push(
            getDownloadURL(
              ref(this.storage, 'placeholder/operator-cover-photo.png')
            )
          );
        }
        Promise.all(promises).then((e: any) => {
          ...
          setDoc(doc(this.db, 'users', user.user.uid), newUser).then(
            (value) => {
              this.loadingFlag = false;
  ...
  changeRoles(role: string) {
    ...
    switch (role) {
      ...
      case 'operator':
        ...
        break;
      case 'student':
        this.swiperFormStudentInstance.slideTo(1);
        this.role = 'student';
