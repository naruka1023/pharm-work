...
import {
  coverPhotoLoadSuccessful,
  setCurrentUser,
  updateCoverPhoto,
  updateCropProfilePicture,
  updateProfilePicture,
  upgradeToPharma,
} from 'src/app/state/actions/users.action';
import {
  profileHeaderJobPost,
  profileHeaderOperator,
  profileHeaderPharma,
} from '../../model/typescriptModel/header.model';
import {
  AppState,
  Follow,
  filterConditions,
...
export class ProfileheaderComponent implements AfterViewInit {
  ...
  coverListenerObservable: Observable<string> =
    this.coverListenerSubject.asObservable();
  file!: File;
  fileIdentity!: File;
  fileLicense!: File;
  id!: string;
  ...
  ngOnInit() {
    ...
    document
      .getElementById('studentUpgradeModal')
    ...
    switch (this.profileType) {
      case 'job-post':
        ...
        this.categorySymbol =
          this.route.snapshot.queryParamMap.get('categorySymbol')!;
        ...
        this.store
          .select((state: any) => {
            ...
            switch (this.pageSrc) {
              ...
              case 'request-jobs':
                newJob =
                  state.jobpost.JobRequests[this.id + '-' + state.user.uid]
                    .JobPost;
                break;
              case 'notification':
              ...
              case 'bookmark':
                newJob =
                  state.jobpost.Bookmarks[this.id + '-' + state.user.uid]
                    .JobPost;
                break;
              case 'operator-page':
                if (
                  this.route.snapshot.queryParamMap.get('operatorExistFlag') !==
                  null
                ) {
                  let flag =
                  ...
                  if (flag) {
                    ...
                    let operatorUID =
                      this.route.snapshot.queryParamMap.get('operatorUID');
                    let filter: filterConditions = state.jobpost.JobPost.find(
                      (filter: filterConditions) => {
                        return filter.CategorySymbol === categorySymbol;
                      }
                    );
                    newJob = filter.content.find(
      ...
      case 'operator-profile':
        ...
        this.operatorExistFlag =
          this.route.snapshot.queryParamMap.get('operatorExistFlag') == 'true'
            ? true
            : false;
        ...
        this.store
          .select((state: any) => {
            if (this.followerFlag) {
              return state.jobpost.Follows[
                this.userUID + '-' + this.operatorUID
              ].user;
            }
            ...
            if (this.operatorExistFlag) {
              let categorySymbol = 'BA';
              return state.jobpost.JobPost.find(
          ...
          .select((state: any) => {
            ...
            if (this.operatorExistFlag) {
              let operatorUID =
                this.route.snapshot.queryParamMap.get('operatorUID');
              let categorySymbol = 'BA';
              operatorJobs = state.jobpost.JobPost.find(
                (filterCondition: filterConditions) => {
                  return filterCondition.CategorySymbol == categorySymbol;
                }
        ...
        this.headerInformation = this.store.select((state: any) => {
          ...
          let result = {
            ...
            cropProfilePictureUrl:
              this.profileInformation$.cropProfilePictureUrl,
    ...
    this.followFlag$ = this.store.select((state: any) => {
      ...
      if (this.userUID !== '') {
        ...
        let follow: Follow =
          newState.Follows[this.userUID + '-' + this.operatorUID];
        if (follow === undefined) {
          flag = false;
        } else {
    ...
    this.coverListenerObservable.subscribe((cmd: string) => {
      switch (cmd) {
        ...
        case 'upload':
          ...
          ele.removeEventListener('mousedown', mouseDownHandler);
          uploadBytes(
            ref(this.storage, 'users/' + this.userUID + '/cover-photo'),
            this.file
          ).then(() => {
            if (
              this.result.coverPhotoPictureUrl.indexOf('placeholder') !== -1
            ) {
              let user: Partial<User> = {
                uid: this.userUID,
                coverPhotoOffset: ele.scrollTop,
              };
              getDownloadURL(
                ref(this.storage, 'users/' + this.userUID + '/cover-photo')
              ).then((url: string) => {
                ...
                this.userService.updateUser(user).then(() => {
                  ...
                  this.store.dispatch(
                    updateCoverPhoto({
                      coverPhotoPictureUrl: url,
                      offset: ele.scrollTop,
                    })
                  );
              ...
              let userr: Partial<User> = {
                uid: this.userUID,
                coverPhotoPictureUrl:
                  this.result.coverPhotoPictureUrl.split('?t=')[0] +
                  '?t=' +
                  new Date().getTime(),
                coverPhotoOffset: ele.scrollTop,
              };
              ...
              this.userService.updateUser(userr).then(() => {
                ...
                this.store.dispatch(
                  updateCoverPhoto({
                    coverPhotoPictureUrl: userr.coverPhotoPictureUrl!,
                    offset: ele.scrollTop,
                  })
                );
                this.editCoverPhotoFlag = true;
        ...
        case 'save':
          ...
          this.userService.updateUser(user).then(() => {
            ...
            this.store.dispatch(
              updateCoverPhoto({
                coverPhotoPictureUrl: user.coverPhotoPictureUrl!,
                offset: ele.scrollTop,
              })
            );
            this.editCoverPhotoFlag = true;
    ...
    switch (this.profileType) {
      ...
      case 'pharmacist-profile':
        if (this.photoFlag !== 'unset') {
          ...
          document.getElementById('coverPhoto')!.scrollTop =
            this.coverPhotoVerticalPosition;
        }
        break;
    }
  }
    ...
    if (this.requestChangeForm.invalid) {
      ...
      if (!this.needIDFlag) {
        ...
        reader.onload = (event2) => {
          ...
          toSend = {
            subject:
              this.result.name + ' ' + this.result.surname + ' request change',
            html:
              this.result.name +
              ' ' +
              this.result.surname +
              '  license: ' +
              this.requestChangeForm.value.license +
              ' uid: ' +
              this.result.uid,
            attachments: [
              {
          ...
          this.userService
            .sendRequestChangeEmail(JSON.stringify(toSend))
            .subscribe((email) => {
              this.userService
                .updateUser({
                  uid: this.result.uid,
                ...
                .then(() => {
                  ...
                  this.store.dispatch(
                    setCurrentUser({ user: { requestChangeStatus: 'pending' } })
                  );
              ...
              .subscribe((email) => {
                this.userService
                  ...
                  .then(() => {
                    ...
                    this.store.dispatch(
                      setCurrentUser({
                        user: { requestChangeStatus: 'pending' },
                      })
                    );
                    this.changeStage('pending');
                  });
  ...
  authenticateLicense() {
    ...
    this.userService
      .authenticateLicense(
        this.profileInformation$.name,
        this.profileInformation$.surname,
        this.upgradeToPharmaForm.value.license
      )
      .subscribe((response: any) => {
        if (response.result) {
          this.userService
            .upgradeToPharma(
              this.profileInformation$.uid,
              this.upgradeToPharmaForm.value.license
            )
            .then(() => {
              ...
              this.store.dispatch(
                upgradeToPharma({
                  license: this.upgradeToPharmaForm.value.license,
                })
              );
  ...
  resetFormGroup() {
    this.initializeFormGroup();
    this.introTextForm.patchValue({
      introText: this.result.introText,
      nickName: this.result.nickName,
    });
  }
  ...
  uploadProfilePicture() {
    ...
    if (!this.editExistingPhoto) {
      uploadBytes(
        ref(this.storage, 'users/' + this.userUID + '/profile-picture'),
        ...
        if (this.result.profilePictureUrl.indexOf('placeholder') !== -1) {
          getDownloadURL(
            ...
            this.userService.updateUser(user).then(() => {
              this.store.dispatch(
                updateProfilePicture({ profilePictureUrl: url })
              );
            });
          });
        }
    ...
    const storageRef = ref(
      this.storage,
      'users/' + this.userUID + '/crop-profile'
    );
    listAll(ref(this.storage, 'users/' + this.userUID)).then((list: any) => {
      croppedNotExists =
        list.items.find((item: any) => item.name === 'crop-profile') ==
        undefined;
      ...
      uploadString(storageRef, imgSrc, 'data_url').then(() => {
        if (croppedNotExists) {
          getDownloadURL(storageRef).then((url: string) => {
            let user: Partial<User> = {
              uid: this.userUID,
              cropProfilePictureUrl: url,
            };
            this.userService.updateUser(user).then(() => {
              this.loadProfilePictureFlag = false;
              this.store.dispatch(
                updateCropProfilePicture({ cropProfilePictureUrl: url })
              );
              this.formModal.hide();
            });
          });
          ...
          profPic.src =
            this.result.cropProfilePictureUrl.split('?t=')[0] +
            '?t=' +
            new Date().getTime();
          let user: Partial<User> = {
            uid: this.userUID,
            cropProfilePictureUrl: profPic.src,
          };
          this.userService.updateUser(user).then(() => {
            this.store.dispatch(
              updateCropProfilePicture({ cropProfilePictureUrl: profPic.src })
            );
            this.loadProfilePictureFlag = false;
            this.formModal.hide();
          });
  ...
  editProfilePictureClick($event: any, editExistingPhoto?: string) {
    ...
    if (editExistingPhoto === '') {
      ...
      let reff: any = ref(
        this.storage,
        `users/${this.result.uid}/profile-picture`
      );
      ...
      self.cropper = new Cropper(myImg, {
        ...
        ready: function (event) {
          let item: any = document
            .getElementsByClassName('cropper-view-box')
            .item(0);
          let item2: any = document
            .getElementsByClassName('cropper-face')
            .item(0);
  ...
  repositionGuard(event: any) {
    ...
    something.src = (window.URL ? URL : webkitURL).createObjectURL(
      ...
      this.jobPostService.followOperator(follower).then((response: any) => {
        if (this.operator != undefined) {
          ...
          this.landingPageComponent.appendAlertfromOutside({
            body: '',
            image:
              this.operator.cropProfilePictureUrl !== undefined
                ? this.operator.cropProfilePictureUrl
                : this.operator.profilePictureUrl,
          ...
          this.jobPostService
            .getOperatorFromUID(this.operatorUID)
            .then((responseOperator) => {
              this.followedText = 'ติดตามแล้ว';
              ...
              this.landingPageComponent.appendAlertfromOutside({
                body: '',
                image:
                  newOperator.cropProfilePictureUrl !== undefined
                    ? newOperator.cropProfilePictureUrl
                    : newOperator.profilePictureUrl,
                url: 'empty',
